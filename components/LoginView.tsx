...
import { 
  ...
  Building2, Building, Siren, Cloud, Key, ExternalLink, BadgeCheck
} from 'lucide-react';
      ...
      <div className="w-full max-w-6xl relative z-10">
        {step === 1 ? (
          <div className="animate-in fade-in slide-in-from-bottom-12 duration-1000 flex flex-col items-center">
            ...
            {/* GOOGLE CLOUD LINK PANEL */}
            <div className="w-full max-w-3xl mb-12 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-8 rounded-[3rem] border border-slate-200 dark:border-white/10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
              <div className="flex items-center gap-5">
                <div className={`p-4 rounded-2xl transition-all ${isGcpLinked ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/40' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                   <Cloud className={`w-8 h-8 ${isGcpLinked ? 'animate-pulse' : ''}`} />
                </div>
                <div>
                   <h4 className="text-lg font-black uppercase italic text-slate-900 dark:text-white leading-none">Google Cloud Core</h4>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">
                     {isGcpLinked ? 'Linked to High-Reasoning Project' : 'Connect GCP Project for Neural Triage'}
                   </p>
                </div>
              </div>
                ...
                <button 
                  ...
                  className={`px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all flex items-center gap-3 ${
                    isGcpLinked 
                      ...
                      : 'bg-blue-600 hover:bg-blue-700 text-white shadow-xl hover:scale-105 active:scale-95'
                  }`}
                >
                  {isGcpLinked ? <ShieldCheck className="w-4 h-4" /> : <Key className="w-4 h-4" />}
                  {isGcpLinked ? 'Project Connected' : 'Link Google Cloud'}
                </button>
                {!isGcpLinked && (
                  <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="text-[8px] font-bold text-slate-400 hover:text-blue-500 flex items-center gap-1 uppercase tracking-widest">
                    Requires Paid Billing Project <ExternalLink className="w-2 h-2" />
                  </a>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10 w-full max-w-5xl">
              {[
                ...
                <button
                  key={item.role}
                  onClick={() => { setSelectedRole(item.role); setStep(2); setName(''); setExtra(''); setSecondaryExtra(''); setHospitalType(''); setLicense(''); setError(null); }}
